...
function renderAtlasSummary(total, shown, matched, query) {
  ...
  if (query) {
    atlasSummary.textContent =
      matched > shown
        ? `已索引 ${total.toLocaleString("zh-CN")} 个 OSEA 标签，找到 ${matched.toLocaleString("zh-CN")} 个结果，先显示前 ${shown} 个。${sourceNote}`
        : `已索引 ${total.toLocaleString("zh-CN")} 个 OSEA 标签，找到 ${matched.toLocaleString("zh-CN")} 个结果。${sourceNote}`;
    return;
  }

  atlasSummary.textContent = `已索引 ${total.toLocaleString("zh-CN")} 个 OSEA 标签；默认展示已补充图文的常见鸟和部分基础标签。${sourceNote}`;
}
...
async function renderBirds(options = {}) {
  ...
  try {
    ...
    renderAtlasSummary(entries.length, visible.length, matched.length, query);
